...
def main(window_size=1000):
    ...
    colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:purple']  # blackscholes, vips, dedup, ferret
    ...
    for i, (exp_name, config) in enumerate(configs.items()):
        ...
        # Process each run
        for run in range(3):
            log_file = os.path.join(os.path.dirname(__file__), f"4_1_a_c_logs_run1", f"{exp_name}_run{run}.txt")
        ...
        avg_qps, avg_latency, avg_latency_std = aggregate_qps_data(avg_qps, avg_latency, std_latency, window_size)
        
        
       
        # Plot the data with error bars and improved visibility
        plt.errorbar(avg_qps, avg_latency, yerr=avg_latency_std, 
    ...
    plt.figtext(0.5, 0.01, f'Note: Data averaged across 3 runs, aggregated in {window_size} QPS windows, error bars show standard deviation', 
