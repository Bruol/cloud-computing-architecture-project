...
def main():
    ...
    for i, (exp_name, config) in enumerate(configs.items()):
        ...
        for run in range(3):
            # Read mcperf logs
            log_file = os.path.join(os.path.dirname(__file__), f"4_1_d_logs", f"{exp_name}_run{run}.txt")
            mcperf_log = mcPerfLogs.McPerfLogs(log_file)
            data = mcperf_log.parse_log_file()
            
            # Read CPU usage data
            cpu_file = os.path.join(os.path.dirname(__file__), f"4_1_d_logs", f"cpuUsage{exp_name.replace('experiment', '')}_run{run}.csv")
